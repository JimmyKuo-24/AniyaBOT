...
def cash_exrate_sixMonth(code1):
    ...
    if currency['現金買入'][0] == '-' or currency['現金買入'][0] == 0.0:
        return "現金匯率無資料可分析"
    currency.plot(kind='line', x='Date', y=[u'現金買入', u'現金賣出'], figsize=(12, 6), fontsize=12, color=['blue', 'orange'])
...
def spot_exrate_sixMonth(code2):
    ...
    if currency['即期買入'][0] == '-' or currency['即期買入'][0] == 0.0:
        return "即期匯率無資料可分析"
    currency.plot(kind='line', x='Date', y=[u'即期買入', u'即期賣出'], figsize=(12, 6), fontsize=12, color=['blue', 'orange'])
    plt.legend(prop=chinese_font)
